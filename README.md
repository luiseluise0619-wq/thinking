...
```
- 오늘의 훈련(빠른 판단 5분 / 깊은 훈련) · 사고 프레임 · 멘탈 모델 라이브러리 · 성장 대시보드(Thinking Genome)
- **AI 코치 3모드**:
  - **소크라테스** — 고민에 답 대신 파고드는 질문 6개
  - **논쟁** — 주장을 5개 입장에서 반박
  - **가이드 세션** — 사고법 하나(원칙·소크라테스·시스템·역발상·2차·확률적)로 질문을 하나씩 던지며 끝까지 이끈 뒤, 사고 과정 분석 → 현실 비교 → 6축 채점(논리성·근거·반례·유연성·현실성·창의성) → 개선 미션
- **성장은 '출석'이 아니라 '사고 품질'로** — XP·능력치·레벨이 세션의 측정된 사고 깊이·본질·반례·근거·실행에 비례한다(반복만으론 오르지 않음).
- **사고력 진단(효과 검증)** — 훈련 전 사전 진단으로 기준선을 재고, 훈련 후 동일 문항으로 사후 진단해 **실제 변화**(반증·반례·근거·대안·2차·본질 6축 + 사고력 지수)를 숫자로 보여준다. 결정론적 규칙 채점이라 사전·사후가 공정하게 비교된다.
