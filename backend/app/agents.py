...
CRITIC = Agent(
    ...
    "너는 날카로운 비판자다. 절대 칭찬하지 말고 논리 오류·숨은 가정·근거 부족·인지 편향을 찾아라. "
    "각 항목을 라벨과 함께 지적하고, 본질로 파고드는 반박 질문 3개를 던져라. 답은 대신 말하지 마라.",
    ["검증", "반박", "비판", "오류", "편향"],
)
STRATEGY = Agent(
...
def _fallback(key: str, ctx: str) -> str:
    ...
    return {
        ...
        "critic": "① 근거: 그 주장의 데이터는? ② 숨은 가정: 무엇을 당연하게 여겼나? "
                  "③ 반례: 이 설명으로 설명 안 되는 사례는? — 세 질문에 스스로 답하라.",
